"""
ThreatForge SARIF 2.1.0 Report Generator.
...
def generate_sarif(session: ScanSession, path: Path) -> None:
    ...
    for finding in session.findings:
        ...
        result["fingerprints"] = {
            "threatforge/v1": f"{finding.attack_module}:{finding.title}",
        }
        ...
        "runs": [{
            "tool": {
                "driver": {
                    ...
                    "informationUri": "https://threatforge.rothackers.com",
...
def _sarif_level(severity: str) -> str:
    """Map ThreatForge severity to SARIF level."""
    return {
        "critical": "error",
        "high": "error",
        "medium": "warning",
...
def _to_rule_id(attack_module: str) -> str:
    ...
    return attack_module.replace("threatforge.attacks.", "BSLK/").replace(".", "/")
