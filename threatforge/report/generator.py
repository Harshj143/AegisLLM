"""
ThreatForge Report Generator — multi-format vulnerability reporting.
...
async def generate_report(session: ScanSession, output_config: OutputConfig) -> str:
    ...
    base_name = f"threatforge_{session.id}_{timestamp}"
    ...
    if fmt == "json":
        ...
        from threatforge.report.sarif import generate_sarif
        ...
        from threatforge.report.pdf import generate_pdf
...
def _write_json_report(session: ScanSession, path: Path) -> None:
    """Write a JSON report."""
    report = {
        "threatforge_version": __version__,
...
def _write_markdown_report(session: ScanSession, path: Path) -> None:
    ...
    lines.extend([
        "",
        "---",
        f"*Generated by ThreatForge v{__version__} — AI Red Teaming Framework | [Rot Hackers](https://rothackers.com)*",
    ])
