"""
ThreatForge Session — manages the lifecycle of a single scan.
...
from threatforge.core.database import ThreatForgeDatabase
...
class ScanSession:
    """
    Manages state for a single ThreatForge scan.
    ...
    def __init__(
        self,
        config: ThreatForgeConfig,
        session_id: str | None = None,
    ) -> None:
        self.id = session_id or uuid.uuid4().hex[:12]
        self.config = config
        self.profile = ThreatForgeProfile(
        ...
        self._db: ThreatForgeDatabase | None = None
        self._event_listeners: list[Callable[..., Any]] = []

    async def initialize(self) -> None:
        """Connect to database and prepare session."""
        self._db = ThreatForgeDatabase(self.config.session_db)
    ...
    @classmethod
    async def resume(cls, session_id: str, db_path: str = "./threatforge-sessions.db") -> ScanSession:
        """Resume an interrupted scan session from the database."""
        db = ThreatForgeDatabase(db_path)
        await db.connect()
        ...
        config = ThreatForgeConfig.from_cli_args(**session_data.get("config", {}))
        session = cls(config, session_id=session_id)
        session._db = db
        session.status = "resumed"

        if session_data.get("profile"):
            session.profile = ThreatForgeProfile.from_dict(session_data["profile"])
