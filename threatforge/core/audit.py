"""
ThreatForge Audit Logger — forensic logging for all scan operations.
...
logger = logging.getLogger("threatforge.audit")


class AuditLogger:
    """
    Append-only audit logger for ThreatForge scan operations.
    ...
    verification. Enabled by default — set THREATFORGE_AUDIT=0 to disable.
    ...
    def __init__(
        ...
        # Check env override: THREATFORGE_AUDIT=0 disables
        if enabled is None:
            enabled = os.environ.get("THREATFORGE_AUDIT", "1") != "0"

        self.enabled = enabled
        self._session_id = session_id
        
        # Security: Use a secret for HMAC signatures to prevent tamper-and-recalculate attacks
        self._audit_secret = os.environ.get("THREATFORGE_AUDIT_SECRET")
        ...
        if self.enabled:
            ...
            self._write_entry("session_start", {
                ...
                "threatforge_version": _get_version(),
                "pid": os.getpid(),
                "cwd": os.getcwd(),
            })
...
def _get_version() -> str:
    """Get threatforge version."""
    try:
        from threatforge import __version__
        return __version__
    except ImportError:
        return "unknown"
