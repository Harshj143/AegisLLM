"""
ThreatForge Scan — orchestrates the full scan pipeline.
...
from threatforge.core.audit import AuditLogger
from threatforge.providers.base import ProviderMessage
from threatforge.providers.litellm_adapter import LiteLLMAdapter
from threatforge.providers.custom_http import CustomHTTPAdapter
from threatforge.providers.websocket import WebSocketAdapter

console = Console()
logger = logging.getLogger("threatforge")
...
async def run_scan(
    ...
    output_dir: str = "./threatforge-reports",
    ...
    async with _create_provider(cfg) as prov:
        ...
        console.print(Panel(
            ...
            title="⚔️  ThreatForge Scan Started",
            border_style="red",
            padding=(1, 2)
        ))
        ...
        from threatforge.report.generator import generate_report
...
def _create_provider(cfg: ThreatForgeConfig):
...
async def _run_recon(prov, session: ScanSession) -> None:
    ...
    from threatforge.recon.guardrails import profile_guardrails
    from threatforge.recon.tools import discover_tools
    from threatforge.recon.context import measure_context_window
...
async def _run_evolution(prov, session: ScanSession, cfg: ThreatForgeConfig) -> None:
