"""
ThreatForge CLI — Main entry point with click + rich.

Commands:
  threatforge scan      — Run a full scan against a target
  threatforge recon     — Reconnaissance only
  threatforge diff      — Differential scan across multiple models
  threatforge posture   — Guardrail posture assessment (recon-only)
  threatforge replay    — Replay a previous session
  threatforge modules   — List available attack modules
  threatforge version   — Show version
"""
...
from threatforge import BANNER, __version__

console = Console()


@click.group()
@click.version_option(version=__version__, prog_name="threatforge")
def cli() -> None:
    """🐍 ThreatForge — LLM/AI Red Teaming Framework"""
    pass
...
@click.option("--output-dir", default="./threatforge-reports", help="Report output directory")
...
def scan(target, provider, model, api_key, auth, mode, evolve, generations, module, recon_module, attacker_provider, attacker_model, attacker_api_key, output, output_dir, no_dashboard, fail_on, verbose, debug, skip_recon, config) -> None:
    ...
    from threatforge.cli.scan import run_scan
...
def recon(target, provider, api_key, auth, recon_module, verbose) -> None:
    ...
    from threatforge.cli.scan import run_recon
...
@click.option("--db", default="./threatforge-sessions.db", help="Session database path")
def replay(session_id, db) -> None:
    ...
    from threatforge.cli.scan import replay_session
...
def list_modules() -> None:
    ...
    table = Table(title="ThreatForge Attack Modules", show_lines=True)
...
def interactive(target, provider, model, api_key, auth, verbose) -> None:
    ...
    from threatforge.cli.interactive import run_interactive
...
def sessions(db) -> None:
    ...
    from threatforge.cli.replay import list_sessions
...
@click.option("-o", "--output-dir", default="./threatforge-reports", help="Report output directory")
@click.option("-v", "--verbose", is_flag=True)
def diff(target, api_key, category, output_dir, verbose) -> None:
    ...
    if len(targets) < 2:
        ...
        console.print("[dim]Usage: threatforge diff -t openai:gpt-4 -t anthropic:claude-3-5-sonnet-20241022[/dim]")
        return
...
def posture(target, provider, model, api_key, auth, output_dir, json_output, verbose) -> None:
    ...
    from threatforge.posture import run_posture_scan, print_posture_report, save_posture_report
...
@cli.command("version")
def version() -> None:
    """Show ThreatForge version and system info."""
    console.print(BANNER, style="bold red")
...
def main() -> None:
    """Entrypoint for the threatforge CLI."""
    cli()
