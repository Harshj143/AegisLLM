"""
ThreatForge Desktop Backend — FastAPI server for Electron IPC.
...
# Add parent to path for threatforge imports
sys.path.insert(0, str(Path(__file__).parent))

from threatforge.core.config import ThreatForgeConfig
from threatforge.core.session import ScanSession
from threatforge.core.finding import Severity



logger = logging.getLogger("threatforge.desktop")

# Authentication Token (passed from Electron main process)
THREATFORGE_TOKEN = os.environ.get("THREATFORGE_TOKEN")

async def verify_token(x_threatforge_token: str = Header(None)):
    if THREATFORGE_TOKEN and x_threatforge_token != THREATFORGE_TOKEN:
        raise HTTPException(status_code=401, detail="Unauthorized: Invalid ThreatForge Token")
...
app = FastAPI(
    title="ThreatForge Desktop Backend",
    version="1.0.6",
    docs_url="/docs" if os.environ.get("THREATFORGE_DEBUG") else None,
    lifespan=lifespan,
)
...
async def native_status():
    try:
        from threatforge.native_bridge import native_status as get_status
        return get_status()
    except ImportError:
...
async def start_scan(config: ScanConfig):
    """Start a new scan."""
    try:
        cfg = ThreatForgeConfig.from_cli_args(
...
async def generate_report(session_id: str, req: ReportRequest):
    ...
    try:
        ...
        # Generate report
        from threatforge.report.generator import generate_report as gen
        from threatforge.core.config import OutputConfig
        output_cfg = OutputConfig(format=req.format, output_dir="./threatforge-reports")
...
async def websocket_endpoint(ws: WebSocket, token: str = Query(None)):
    await ws.accept()
    if THREATFORGE_TOKEN and token != THREATFORGE_TOKEN:
...
async def _run_scan_background(session: ScanSession, cfg: ThreatForgeConfig):
    ...
    try:
        ...
        # Create provider
        from threatforge.cli.scan import _create_provider, _run_recon
        ...
        if cfg.evolution.enabled and cfg.mode.value in ("standard", "deep", "chaos"):
            ...
            if not seed_payloads:
                from threatforge.attacks.injection.direct import PAYLOADS
            ...
            async def on_bt(individual, gen):
                from threatforge.core.finding import Finding, Severity, AttackCategory
                logger.info(f"Evolution BREAKTHROUGH at Gen {gen}!")
                finding = Finding(
                    title=f"Evolution Breakthrough — Gen {gen}",
                    severity=Severity.HIGH,
                    category=AttackCategory.PROMPT_INJECTION,
                    attack_module="threatforge.evolution",
...
async def start_diff_scan(config: DiffConfig):
    ...
    try:
        ...
        from threatforge.differential import run_differential
...
async def start_posture_scan(config: PostureConfig):
    ...
    try:
        ...
        from threatforge.cli.scan import _create_provider
...
async def get_audit_log(session_id: str):
    ...
    import glob
    logs = glob.glob(f"./threatforge-reports/audit_{session_id}_*.jsonl")
    if not logs:
...
async def list_providers():
    ...
    providers = [
        ...
        {"id": "custom", "name": "Custom HTTP", "models": [], "env_var": "THREATFORGE_API_KEY"},
    ]
...
async def list_mutations():
    ...
    from threatforge.evolution.operators import ALL_OPERATORS
    return {
        "mutations": [
            {
                "name": op.name,
...
def main():
    parser = argparse.ArgumentParser(description="ThreatForge Desktop Backend")
